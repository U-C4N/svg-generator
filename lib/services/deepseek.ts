import { DEEPSEEK_API_KEY, API_CONFIG, SVG_SYSTEM_PROMPT } from "../config"
...
export async function generateSvgWithDeepseek(prompt: string): Promise<string> {
  try {
    ...
    if (!svgContent.startsWith("<svg") || !svgContent.endsWith("</svg>")) {
      throw new Error("Invalid SVG content received from Deepseek")
    }

    return svgContent
  } catch (error) {
    console.error("Error generating SVG with Deepseek:", error)
    throw error
  }
} 